...
function ElectricConfigurator({configurationChanged}) {
    ...
    const sendConfiguration = () => configurationChanged({
        trunk, range, consumption, supercharge, quality, practicality, look, price, volume
    })
    ...
    return (<div className="configurator">
        <section className="goods">
            <h4>Avantages</h4>
            <TextField label="Coefficient Coffre" value={trunk} onChange={event => {
                setTrunk(+event.target.value)
                sendConfiguration()
            }}
                       type="number" size="small" inputProps={{step: 0.5}}
                       margin="dense"/>
            <TextField label="Coefficient Autonomie" value={range} onChange={event => {
            ...
            <TextField label="Coefficient Quality" value={quality}
                       ...
                       inputProps={{step: 0.5}} margin="dense"/>
            <TextField label="Coefficient Apparence" value={look} onChange={event => {
        ...
        <section className="bads">
            <h4>Inconvénients</h4>
            <TextField label="Coefficient Prix" value={price} onChange={event => {
                setPrice(+event.target.value);
                sendConfiguration()
            }}
                       type="number" size="small" inputProps={{step: 0.5}}
                       margin="dense"/>
            <TextField label="Coefficient Volume" value={volume} onChange={event => {
